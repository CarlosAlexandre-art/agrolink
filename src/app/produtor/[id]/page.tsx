...
export default async function PerfilProdutorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const servicosConcluidos = produtor.services.length
  const tiposContratados = [...new Set(produtor.services.map(s => s.tipo))]
  const membroDesde = new Date(produtor.user.createdAt).getFullYear()
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0">
              {produtor.user.avatarUrl ? (
                <img
                  src={produtor.user.avatarUrl}
                  alt={produtor.user.nome}
                ...
                <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center text-3xl font-bold text-green-700">
                  {produtor.user.nome[0].toUpperCase()}
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="font-bold text-gray-800 text-xl">{produtor.user.nome}</h1>
              {produtor.nomeFazenda && (
                <p className="text-sm text-green-700 font-medium mt-0.5">🌾 {produtor.nomeFazenda}</p>
              )}
              {(produtor.user.cidade || produtor.user.estado) && (
                <p className="text-sm text-gray-500 mt-0.5">
                  📍 {[produtor.user.cidade, produtor.user.estado].filter(Boolean).join(', ')}
                </p>
              )}
              {produtor.totalAvaliacoes > 0 && (
                <div className="mt-2">
                  <Estrelas nota={produtor.avaliacao} total={produtor.totalAvaliacoes} />
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm text-center">
            <div className="text-2xl font-bold text-green-700">
              {produtor.totalAvaliacoes > 0 ? produtor.avaliacao.toFixed(1) : '—'}
            </div>
